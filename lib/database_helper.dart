...
class DatabaseHelper {
  ...
  Future<Database> initDatabase() async {
    return openDatabase(
      ...
      onCreate: (db, version) {
        db.execute(
          'CREATE TABLE todos(id TEXT PRIMARY KEY, text TEXT, description TEXT, imagePath TEXT, createdDate TEXT, targetCompletionDate TEXT, done INTEGER, isSelected INTEGER)',
        );
        db.execute(
          'CREATE TABLE archived_todos(id TEXT PRIMARY KEY, text TEXT, description TEXT, imagePath TEXT, createdDate TEXT, targetCompletionDate TEXT, done INTEGER, isSelected INTEGER)',
        );
      },
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 3) {
          await db.execute('ALTER TABLE todos ADD COLUMN isSelected INTEGER DEFAULT 0');
        }
        if (oldVersion < 4) {
          await db.execute('ALTER TABLE archived_todos ADD COLUMN isSelected INTEGER DEFAULT 0');
        }
        // Add additional upgrade steps if needed for future versions
      },
      version: 5,
    );
  }
